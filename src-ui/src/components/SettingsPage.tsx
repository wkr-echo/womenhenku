...
export function SettingsPage() {
  ...
  const sections = [
    { key: "provider", label: "AI 服务" },
    { key: "agent", label: "Agent 参数" },
  ...
  return (
    <div className="flex-1 flex overflow-hidden">
      ...
      <div className="w-[200px] border-r border-[var(--border-color)] py-4 bg-[var(--bg-secondary)]">
        <h2 className="px-5 text-sm font-semibold mb-3">设置</h2>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--text-tertiary)] mt-1">
            配置 OpenAI 兼容的 API 服务，支持云端和本地大模型
          </p>
        </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-[var(--text-tertiary)]">
                    ...
                    <span>思考模型：{p.thinking_model}</span>
                  </div>
                </div>
      ...
      {showAdd && (
        <div className="mt-4 rounded-xl border border-[var(--border-color)] bg-[var(--bg-secondary)] p-4">
          <h4 className="font-medium text-sm mb-3">新建 Provider</h4>
          <div className="grid grid-cols-2 gap-3">
            <Input placeholder="名称（如 Ollama）" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            <Input placeholder="Base URL（如 http://localhost:11434/v1）" value={form.base_url} onChange={(e) => setForm({ ...form, base_url: e.target.value })} />
            <Input placeholder="API Key（可选）" type="password" value={form.api_key} onChange={(e) => setForm({ ...form, api_key: e.target.value })} />
            <Input placeholder="默认模型" value={form.default_model} onChange={(e) => setForm({ ...form, default_model: e.target.value })} />
            <Input placeholder="思考模型" value={form.thinking_model} onChange={(e) => setForm({ ...form, thinking_model: e.target.value })} />
          </div>
...
function AgentSettings() {
  ...
  return (
    <div>
      <h3 className="text-lg font-semibold mb-1">Agent 参数配置</h3>
      <p className="text-sm text-[var(--text-tertiary)] mb-6">配置 AI 摘要和翻译的默认参数</p>

      <div className="space-y-6 max-w-md">
        <div>
          <label className="block text-sm font-medium mb-2">摘要目标语言</label>
          <Dropdown
            items={languages}
        ...
        <div>
          <label className="block text-sm font-medium mb-2">摘要详细程度</label>
          <Dropdown
            items={detailLevels}
        ...
        <div>
          <label className="block text-sm font-medium mb-2">翻译并发度：{config.concurrencyDegree}</label>
          <input
            type="range"
            min="1"
            max="5"
        ...
        <div>
          <label className="block text-sm font-medium mb-2">默认 AI 模型</label>
          <Dropdown
            items={[
              { label: "跟随 Provider 默认", value: "" },
...
function AppearanceSettings({ theme, onToggleTheme }: { theme: string; onToggleTheme: () => void }) {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-[var(--text-tertiary)] mb-6">自定义阅读体验</p>
      ...
      <div className="space-y-6 max-w-md">
        ...
        <div>
          <label className="block text-sm font-medium mb-2">阅读字体</label>
          <Dropdown
            items={fonts}
            value="system"
            onChange={() => {}}
        ...
        <div>
          <label className="block text-sm font-medium mb-2">代码字体</label>
          <Dropdown
            items={[
              { label: "JetBrains Mono", value: "JetBrains Mono" },
...
function SyncSettings() {
  return (
    <div>
      <h3 className="text-lg font-semibold mb-1">同步设置</h3>
      <p className="text-sm text-[var(--text-tertiary)] mb-6">配置订阅源自动同步</p>

      <div className="space-y-6 max-w-md">
        <div>
          <label className="block text-sm font-medium mb-2">自动同步间隔</label>
          <Dropdown
            items={[
              { label: "手动同步", value: "0" },
        ...
        <div>
          <label className="block text-sm font-medium mb-2">并发同步数</label>
          <Dropdown
            items={[
              { label: "1", value: "1" },
              { label: "3", value: "3" },
        ...
        <div>
          <label className="block text-sm font-medium mb-2">OPML 导出</label>
  ...
  return (
    <div>
      ...
      <div className="space-y-3 text-sm text-[var(--text-secondary)] max-w-md">
        ...
        <div className="flex justify-between py-2 border-b border-[var(--border-color)]">
          <span className="text-[var(--text-tertiary)]">技术栈</span>
          <span>Tauri 2 + Rust + React + SQLite</span>
        </div>
        <div className="flex justify-between py-2 border-b border-[var(--border-color)]">
          <span className="text-[var(--text-tertiary)]">平台</span>
          <span>Windows / macOS / Linux</span>
        </div>
        <div className="flex justify-between py-2 border-b border-[var(--border-color)]">
          <span className="text-[var(--text-tertiary)]">许可证</span>
          <span>MIT</span>
        </div>
      </div>
    </div>
