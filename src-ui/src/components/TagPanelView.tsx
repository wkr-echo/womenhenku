...
import { getEntryTags, listTags, addTag, tagEntry, untagEntry } from "@/api/feed";
  ...
  return (
    <div style={{
      ...
      <div style={{ borderTop: "1px solid #e5e7eb", paddingTop: 12 }}>
        ...
        <p style={{ fontSize: 11, color: "#dc2626", marginBottom: 4 }}>DEBUG: {JSON.stringify(entryTags.map(t => t.name))}</p>
        {entryTags.length > 0 ? (
          <div>
            {entryTags.map(tag => (
              <button
