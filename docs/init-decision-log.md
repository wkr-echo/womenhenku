...
| 决策点   | 刘欣慧（英文详细版）      | 杜偲妍（Windows 优先版）          | 王康睿                                        |
| -------- | ------------------------- | --------------------------------- | --------------------------------------------- |
...
- 杜偲妍版本 倾向 Swift：可复用 Readability、Agent 状态机、数据库层
...
- 杜偲妍 方案：Swift Sidecar + Tauri IPC（JSON-RPC stdin/stdout）
...
- 确认老师要求"大模型中立，支持任意可提供标准 API 的大模型"
...
| 功能模块                    | 技术栈       | Rust 侧                                                                                                                                                             | React 侧                                                                                             |
| --------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| Feed 订阅与内容管线（①②） | Rust + React | Feed 解析/同步、OPML 导入导出、SQLite schema 与迁移、Readability 提取管线、Markdown 转换、搜索索引                                                                  | 订阅源管理页、文章列表页（已读/未读/分页）、阅读器页（主题/字体/布局）、侧边栏                       |
| AI 智能体（③④）           | Rust + React | Provider 管理、OpenAI 兼容协议封装（`/v1/chat/completions` + SSE 流式）、Summary Agent（可配置语言/详细程度/流式输出）、Translation Agent（段落级并发/重试/清除） | 摘要面板（流式展示）、双语翻译面板（段落对照布局）、Agent 配置页（Provider/Model/Prompt 定制）       |
| 笔记、文摘与基础设施（⑤）  | Rust + React | 笔记 CRUD、文摘导出策略与模板渲染、设置持久化、Tauri Command 契约定义与维护、平台抽象接口                                                                           | 笔记编辑页、文摘导出面板、设置页（通用配置/Prompt 模板编辑）、通用 UI 组件库（布局/主题/键盘快捷键） |

契约先行：三人先协定 Tauri Command 列表（接口契约），然后各自独立开发，互不阻塞。
...
| 人员  | 功能模块                     | 原因                                                                                                |
| ----- | ---------------------------- | --------------------------------------------------------------------------------------------------- |
| 队员A | 项目经理 + AI 智能体（③④） | Rust 异步流式处理最复杂（SSE 解析、并发控制、状态机），需把控核心架构；同时涉及 React 流式渲染      |
| 队员B | Feed 订阅与内容管线（①②）  | 数据处理链路最长（Feed 解析→清洗→Markdown→渲染），涉及多种格式与边界情况，Rust 解析能力强        |
| 队员C | 笔记、文摘与基础设施（⑤）   | 功能相对独立（CRUD + 模板渲染），React 工作量较大（编辑页/设置页/通用组件），成果可视化强，成就感高 |
