...
- **项目名**：womenhenku（Mercury RSS 阅读器的跨平台复刻）
- **仓库**：`https://github.com/wkr-echo/womenhenku.git`（分支 `dev`）
...
**最终正确修复**（commit `62e8896`）：**完全移除 pre 块保护代码**，让 `extract()` 直接把原始 HTML 传给 Readability。然后提升 `PIPELINE_VERSION` 5→6 强制刷新缓存。
...
| # | 任务 | 说明 |
|---|---|---|
| 1 | Feed 错误信息持久化 | 当前 Feed 添加/刷新失败的错误信息只在 Toast 一闪而过，需持久化到数据库或 UI 可查看 |
| 2 | 相对 URL fallback | `parser.rs` 和 `commands.rs` 已有 `Url::join()` 处理，但需确认所有路径覆盖 |
...
# Git（走代理）
cd /home/wkr/womenhenku && git push origin dev
# 代理：127.0.0.1:7897
```
